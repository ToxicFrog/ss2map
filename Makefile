...
ss2:
	./mishtml --gamesys ss2/shockscp.gam --strings ss2/strings --html-out maps/ss2 ss2/{earth,station,eng,medsci,hydro,ops,rec,command,rick,many,shodan}*.mis

# T1: miss8.mis doesn't exist, miss18.mis is an easter egg(?) mission with no brushes
t1:
	./mishtml --gamesys t1/dark.gam --propformat t1 --html-out maps/t1 t1/miss{1..7}.mis t1/miss{9..17}.mis

# T2: miss3.mis doesn't exist (cut partway through development)
t2:
	./mishtml --gamesys t2/dark.gam --propformat t2 --html-out maps/t2 t2/miss{1,2}.mis t2/miss{4..16}.mis
